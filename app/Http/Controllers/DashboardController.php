    ...
    public function index()
    {
        ...
        try {
            ...
            if (Auth::user()->role == 'Administrator') {
                ...
                $evictions = DB::select('select id, property_address, status, file_type, is_downloaded, owner_name, tenant_name, docket_number, court_date, total_judgement, filing_fee,  created_at, is_extra_files, court_number, is_in_person_filing, is_withdrawn from evictions ORDER BY FIELD(status, "Created LTC", "LTC Mailed", "LTC Submitted Online", "Court Hearing Scheduled", "Court Hearing Extended", "Judgement Issued in Favor of Owner", "Judgement Denied by Court", "Tenant Filed Appeal", "OOP Mailed", "OOP Submitted Online", "Paid Judgement", "Locked Out Tenant"), id DESC');
            }
...
<p>CourtZip Team</p>');
    ...
    public function getEvictionNotes(Request $request) {
        try {
            $errorMsg = new ErrorLog();
            $errorMsg->payload = 'shit ' . $request->eviction_id;
            $errorMsg->save();
