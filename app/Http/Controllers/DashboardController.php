        ...
        try {
            ...
            if (Auth::user()->role == 'Administrator' || Auth::user()->role == 'PM Company Leader') {
                $evictions = DB::table('evictions')
                    ->select('evictions.id', 'users.id', 'user_id', 'property_address', 'status', 'file_type', 'is_downloaded', 'owner_name', 'tenant_name', 'court_date', 'total_judgement', 'filing_fee',  'evictions.created_at', 'is_extra_files', 'court_number')
