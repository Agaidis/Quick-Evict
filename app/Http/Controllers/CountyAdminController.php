...
class CountyAdminController extends Controller
{
    ...
    public function getCourtIds(Request $request) {
        try {
            $courtIds = DB::table('court_details')->select('court_number')->where('county' , '=', $request->county)->distinct()->get();
            // CourtDetails::select('court_number')->where('county', $request->county)->orderBy('id', 'DESC')->groupBy('court_number')->get();
    ...
    public function getNotes(Request $request) {
        try {
            $errorMsg = new ErrorLog();
            $errorMsg->payload = $request->courtId;

            $errorMsg->save();
            $updatedCountyNotes = CountyNotes::where('court_id', $request->courtId)->orderBy('id', 'DESC')->get();
    ...
    public function addNote(Request $request) {
        try {
            ...
            CountyNotes::where('id', $newCountyNote->id)
                ->update(['notes' => '<div class="county_note" id="county_'.$newCountyNote->id.'_'.$request->courtId.'"><p style="font-size:14px; margin-bottom:0;"> '.$userName . ' | '. $date . '<span class="fas fa-trash delete_county_note" id="delete_county_note_'.$newCountyNote->id.'_'.$request->courtId.'" style="display:none; cursor:pointer; color:red;"></span></p>' . $request->note .'<hr></div>']);
